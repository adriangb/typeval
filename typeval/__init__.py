...
from typing import Any, Dict, Iterable, Iterator, List, Literal, Mapping, Set, Tuple, Union
...
def _build_schema(tp: type, seen: Set[type], recursive_containers: Set[type]) -> Mapping[str, Any]:
    ...
    if origin is Literal:
        ...
        if len(args) == 2 and None in args:
            ...
            schema["schema"] = _build_schema(next(iter(arg for arg in args if arg is not None)), seen, recursive_containers)
            return schema
        else:
            schema["type"] = "union"
            schema["choices"] = [_build_schema(tp_arg, seen, recursive_containers) for tp_arg in get_args(tp)]
            return schema
    elif isinstance(origin, type) and issubclass(origin, List):
        schema["type"] = "list"
        schema["items"] = _build_schema(next(iter(get_args(tp))), seen, recursive_containers)
